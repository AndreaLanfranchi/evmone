...
// Build constexpr cost lookup table for SSTORE.
constexpr auto sstore_costs = []() noexcept {
    std::array<std::array<StorageCost, EVMC_STORAGE_MODIFIED_RESTORED + 1>, EVMC_MAX_REVISION + 1>
        tbl{};
    ...
    for (size_t rev = EVMC_FRONTIER; rev <= EVMC_MAX_REVISION; ++rev)
    {
        ...
        {
            e[EVMC_STORAGE_ASSIGNED] = {c.reset, 0};
